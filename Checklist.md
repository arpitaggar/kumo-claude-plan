...
- [x] **New migration `docs/supabase_migrations/stage35_org_join_codes.sql`** — `org_join_codes` table (admin-only SELECT RLS, no direct insert/update/delete policy — all mutation goes through RPCs), `org_members.cost_field_option_id` column + cross-org guard trigger, and 3 `SECURITY DEFINER` RPCs: `generate_org_join_code` (admin-only, collision-checked random code), `revoke_org_join_code` (admin-only), `redeem_org_join_code` (any authenticated user; `FOR UPDATE` row lock for concurrency-safe limited-use redemption, successful-joins-per-hour cap, server-side-only org/user resolution — never trusts a client-supplied id). **Not yet run against the live database.**
