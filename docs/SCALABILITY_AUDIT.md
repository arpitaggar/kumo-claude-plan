...
- **Why this shape and not Instagram/Facebook's windowing technique (2026-08-16 addendum):** Discussed directly with the user, who asked whether adopting the fuller Instagram-style technique now would give users a more familiar experience. Conclusion: no — the two approaches solve different problems, and Kumo's feed doesn't have the property that makes Instagram's version pay off.
  - **What Instagram/Facebook actually do:** a *bidirectional* sliding window (evict both above and below the visible region, not just the oldest end), backed by a separately LRU-capped image cache, with **re-fetch-on-scroll-back silently returning different content** — because their feed is rank-based and ephemeral, a re-served page not matching the original is invisible or even expected.
  - **Why that doesn't transfer here:** Kumo's feed is deterministic and cursor-paginated (`created_at`-ordered real posts, not a ranking algorithm) — a scroll-back re-fetch has to return the *same* posts in the *same* order to avoid a visibly worse experience (reshuffled or missing posts), which the current fresh-reload-from-top trade-off already accepts honestly rather than hiding. Copying Instagram's bidirectional/re-serve technique would add real complexity (cursor bookkeeping in both directions, cache-consistency handling for concurrent new posts) to solve a freshness problem Kumo doesn't have, while not actually improving on the deterministic list's one genuine advantage: what's on screen doesn't quietly change on you.
  - **Chosen instead:** the narrower one-directional trim already implemented above — matches Kumo's one-directional "Load more" UX (no infinite scroll in both directions) and keeps the pagination cursor invariant simple.
  - **Revisit if:** (a) real memory complaints or crash reports get traced to feed scrolling — the current 200-post/10-page threshold turns out to be too generous in practice; or (b) the feed itself changes shape — e.g. the product ever moves from a deterministic chronological/cursor feed toward an algorithmically-ranked or ephemeral one (à la Instagram's Explore), at which point re-fetch-on-scroll-back stops being a UX regression and the fuller bidirectional technique becomes worth the added complexity. Neither condition holds today — this is a documented, conscious "not yet," not an oversight.
