...
**✅ Deployed (2026-08-15):** SEC-033 — `stage46_age_gate_db_enforcement.sql` adds a DB-level `require_age_verified()` trigger, and the user has confirmed running it, along with the rest of the `stage42`–`stage45` backlog, against the live database. `stage43` (private buckets) still needs its depending app build to ship in the same release, or an old client gets a 403 on avatar/attachment URLs — a release-process note, not an outstanding migration.
...
| ~~SEC-033 (invite-path age gate is app-side only)~~ | ✅ Resolved and live 2026-08-15 (`stage46_age_gate_db_enforcement.sql`) — no longer deferred, kept in this table only for the historical record |
...
- Every SQL migration through `stage46_age_gate_db_enforcement.sql` is confirmed live against the production database (`stage42`–`stage46`, including the 2026-08-15 SEC-033 DB-enforcement follow-up, confirmed 2026-08-15) ✅
...
- ~~SEC-033~~ and ~~run `stage42`–`stage46` against the live database~~ — both done 2026-08-15, see `docs/SECURITY_AUDIT.md`. `stage43` (private storage buckets) and the app build depending on it still must ship together in the same release
