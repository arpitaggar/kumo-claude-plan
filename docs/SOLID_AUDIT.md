...
9. ✅ **Fixed (2026-08-11).** Blanket-retyped 9 of the 11 flagged repository providers to their abstract interfaces — `packing`, `ratings`, `organization`, `chat`, `itinerary`, `trip_email_alias`, `trip_segment`, `ai_generation`, `expense` (`gamification` was already fixed when found; `auth` is covered separately by priority #3, since that fix is more than a type annotation). Found one genuine bonus while doing it: `chatRepositoryRefProvider` was a second, redundant `Provider<ChatRepositoryImpl>` that existed only because `fetchMessagesBefore` apparently wasn't on the abstract interface at some point — it already is now, so the provider was pure duplication and got deleted; `chat_page.dart` reads `chatRepositoryProvider` directly.
10. ✅ **Fixed (2026-08-11).** `AppConstants`: deleted 6 stale table constants (`usersTable`, `itineraryEventsTable`, `groupsTable`, `groupMembersTable`, `expensesTable`, `messagesTable`) — confirmed zero references anywhere; only `itinerariesTable` was ever actually used. `Validators`: `validateEmail`/`validatePassword` moved to a new `lib/features/auth/domain/validators/auth_validators.dart` (`AuthValidators`) — genuinely auth-domain rules; the rest (`validateNonEmpty`, `validateAmount`, `validateDateRange`, `validateCurrencyCode`, `validatePercentage`, `validateNonEmptyList`, `validateUuid`) stayed in `core/utils` since they're truly generic, not itinerary- or expense-specific despite only being called from those usecases today. `Formatters`: trimmed to the 3 methods actually used anywhere (`formatDate`, `formatDateTime`, `formatCurrency`) — the other 8, including the `formatPhoneNumber` this finding named specifically, had zero call sites and zero test coverage.
