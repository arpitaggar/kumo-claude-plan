...
| `itinerary_detail_page.dart:869-932` vs `add_expense_page.dart:262-281` | Expense-summary recalculation math is **independently reimplemented twice** — a real correctness risk, not just style: the two copies can drift. |
...
- `packing_provider.dart:14`, `rating_provider.dart:17` — repository providers typed to the concrete `Impl` class rather than the abstract interface (latent today, same footgun as auth's, not yet exploited).
...
4. **One clear Clean Architecture breach**: `lib/features/itinerary/domain/entities/trip_theme.dart` imports `flutter/material.dart` and has a `withContext(BuildContext)` method — a domain entity depending on the UI framework, contradicting this project's own CLAUDE.md. Cheap, mechanical fix: move `withContext` to a presentation-layer extension.
...
1. Move `TripTheme.withContext()` out of the domain entity into presentation (only real domain-layer layering breach; trivial fix).
...
5. Unify the two independent expense-summary recalculation implementations (real correctness risk, not just style).
...
9. Blanket-retype `packing`/`ratings` repository providers to their abstract interfaces (cheap, prevents future accidental coupling).
