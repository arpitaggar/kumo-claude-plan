...
2. Redeploy the three Edge Functions (`generate-itinerary`, `invite-email`, `send-message-push`) and set the `ALLOWED_ORIGIN` secret.
3. Ship a Flutter build using `--dart-define-from-file=env.local.json` (see SEC-002) — the old `.env`-asset build path no longer works.
