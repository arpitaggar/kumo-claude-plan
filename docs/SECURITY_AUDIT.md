...
- **2026-08-15 update:** SEC-033 (invite-path age gate enforced app-side only) resolved — `stage46_age_gate_db_enforcement.sql` adds a DB-level `require_age_verified()` trigger, with the enforcement scope and retroactivity decided directly with the user rather than unilaterally (see that entry). A new pre-commit lint (`scripts/check_age_gate_coverage.sh`) future-proofs the same class of gap for tables added later.
...
**Still open:** SEC-014 (Firebase key rotation — manual console action, see Active Remediation Log above). SEC-033 (invite-path age-gate DB enforcement) resolved 2026-08-15 — see that entry; the migration (`stage46_age_gate_db_enforcement.sql`) is code-complete but not yet confirmed run against the live database, per the deployment-gaps list at the top of `docs/Checklist.md`.
