...
- **Open Critical/High Issues:** 0 code-fixable (32 of 34 findings resolved across the 2026-08-05, 2026-08-09, 2026-08-11, and 2026-08-13 remediation passes; SEC-014 remains open pending a manual Firebase console action — key rotation can't be done from a code change — and SEC-033 is a real, verified gap flagged for a product decision rather than fixed unilaterally, see below)
...
- **Severity:** Medium
- **Status:** ⚠️ Open — flagging, not deciding for you. This needs a product/scope call (how much engineering weight to put behind a pre-launch, solo-dev project's compliance mechanism, per `CLAUDE.md`), not a unilateral schema change.
...
- **3. Remediation (not applied here, for the user to weigh):** the durable fix is a database-level check — e.g. a `profiles_require_age_verified` guard (a `BEFORE INSERT/UPDATE` trigger or per-table `WITH CHECK` clause added to the write paths that matter: `itineraries`, `messages`, `itinerary_posts`, etc.) rejecting writes from an authenticated user whose `age_verified_at` is still null, mirroring how `itineraries_owner_all` already enforces `org_id` membership at the database rather than trusting client state. Scope and blast radius (how many tables would need the check, whether it should apply retroactively to already-unverified test/dev rows per the migration's own grandfather clause) is a real design decision, not a one-line patch.
- **4. Resolution Mechanism:** N/A — open.
...
**Still open:** SEC-014 (Firebase key rotation — manual console action, see Active Remediation Log above) and SEC-033 (invite-path age-gate DB enforcement — flagged for a product decision, see that entry).
