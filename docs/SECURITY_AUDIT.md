...
- **Open Critical/High Issues:** 0 code-fixable (33 of 34 findings resolved across the 2026-08-05, 2026-08-09, 2026-08-11, 2026-08-13, and 2026-08-15 remediation passes; SEC-014 remains open pending a manual Firebase console action — key rotation can't be done from a code change)
- **2026-08-15 update:** SEC-033 (invite-path age gate enforced app-side only) resolved and confirmed live — `stage46_age_gate_db_enforcement.sql` adds a DB-level `require_age_verified()` trigger, with the enforcement scope and retroactivity decided directly with the user rather than unilaterally (see that entry), and the user has since run it against the production database. A new pre-commit lint (`scripts/check_age_gate_coverage.sh`) future-proofs the same class of gap for tables added later.
...
- **2026-08-12 legal/privacy audit follow-up:** a separate legal/data-privacy compliance audit (not a security pass) found and fixed four issues outside this doc's original scope: (1) `invite-email`/`inbound-trip-email` were logging raw Resend error-response bodies, which can echo recipient email addresses back into Supabase function logs — both now log only the HTTP status; (2) SEC-012's "Download my data" UI follow-up (see below) is now closed; (3) the privacy policy (both `docs/legal/privacy-policy.html` and the in-app `PrivacyPolicyPage`) claimed a CSV expense-export feature that never existed — corrected to describe the real export mechanism; (4) `trip_email_forward_log.from_address` (a third party's email address) had no retention limit — `stage42_trip_email_log_retention.sql` adds a 90-day purge function, scheduled via `pg_cron` where that extension is enabled.
- **2026-08-12 private storage buckets:** the same audit flagged both `avatars` and `chat-attachments` as `public=true` Storage buckets — readable by anyone with the object URL, no authentication or app-level access control involved, despite `chat-attachments` being a general-purpose file upload that can hold photographed travel documents. `stage43_private_avatars_chat_attachments.sql` flips both to `public=false`: avatars now use the exact same visibility logic as `profiles_select` (owner, or `profile_visibility='public'`, or `is_searchable=true` — i.e. the existing "Public profile"/"Allow others to find me" toggles in Privacy Settings now also govern avatar visibility, no new toggle needed); chat-attachments now requires the caller be a member/owner of the trip the attachment's message belongs to, mirroring `message_attachments_member_read`. Client-side, `getPublicUrl()` output is no longer directly fetchable, so `lib/core/network/signed_storage_url.dart` (`signedStorageUrlProvider`) exchanges it for a signed URL at render time; `lib/shared/widgets/kumo_avatar.dart` (`KumoAvatar`) is the new shared drop-in for the `CircleAvatar(backgroundImage: NetworkImage(...))` pattern used at every avatar call site, and `chat_page.dart`'s attachment bubble/full-screen viewer/file-open sites resolve signed URLs directly. **Deployment note:** this migration and the app build that depends on it must ship together — an old client still calling the pre-signed `getPublicUrl()` result directly will get a 403 once the buckets flip in production.
- **2026-08-13 review — 18+ age gate + Hitchhiker role (`stage44_age_gate.sql`, `stage45_hitchhikers.sql`, both shipped 2026-08-12).** Held to the same "verified, not assumed" bar as every other pass in this doc. Two findings:
  1. **SEC-033 — real gap, flagged not fixed.** The invite-created-account path leaves `profiles.age_verified_at` null until the invitee calls `confirm_age_and_finish_signup()`; `AgeGateNotifier`'s own doc comment calls this "a real authorization gate, not a UI-guidance flag." Grepped `age_verified_at` across all 45 migrations — it is referenced **only** in `stage44_age_gate.sql` itself. No RLS policy or RPC anywhere else checks it, so the gate is currently enforced entirely by the Flutter router's redirect, not the database. See SEC-033 below.
  2. **SEC-034 — no finding, a documented and reasonable decision.** Hitchhiker's token-authenticated RPCs (`hitchhiker_send_message`, `hitchhiker_suggest_item`) have no rate limit, called out in the migration's own comments as a deliberate deferral matching SEC-021's posture. Logged here so it's tracked in this doc's registry, not just in migration comments.
...
- **Category:** Privacy Violation
- **Severity:** Medium
- **Status:** ✅ Resolved (2026-08-05); UI follow-up closed (2026-08-12) — `export_own_data()` RPC (`stage23_security_hardening.sql`) is now wired to a "Download my data" action in `PrivacySettingsPage` (`_downloadMyData`), via `ExportOwnDataUseCase`/`AuthRepository.exportOwnData()`, sharing the result as formatted JSON.
...
### [SEC-033] Age gate for invite-created accounts is enforced app-side only, not by RLS/RPC

- **Category:** Authorization Flaw
- **Severity:** Medium
- **Status:** ✅ Resolved and live (2026-08-15) — `docs/supabase_migrations/stage46_age_gate_db_enforcement.sql`, confirmed run against the production database. Scope/blast-radius call made directly with the user rather than unilaterally: broad per-table trigger guard (not a narrow 3-table version or a "fold into existing RLS policies" version — see that conversation's tradeoff discussion), applied immediately/retroactively (not new-accounts-only), plus a pre-commit lint (`scripts/check_age_gate_coverage.sh`) so a future table can't silently ship ungated.
- **Location:** `docs/supabase_migrations/stage44_age_gate.sql` (no RLS/RPC reference to `age_verified_at` outside this one file — confirmed by grep across all 45 migrations); enforcement lives entirely in `lib/features/auth/presentation/providers/age_gate_provider.dart` + `lib/config/router.dart`'s redirect.
- **1. Cause:** Direct signup is fully server-enforced — `enforce_signup_age_gate()` rejects the `auth.users` INSERT outright for anyone under 18, no account ever created. But an **invite-created** account (`supabase.auth.admin.inviteUserByEmail()`, no DOB available at INSERT time) is created unverified and is only blocked from "real access" by the Flutter router redirecting to `/confirm-age` until `confirm_age_and_finish_signup()` succeeds. Nothing in Postgres — no RLS policy, no other RPC's own check — requires `profiles.age_verified_at is not null` before an authenticated call succeeds.
- **2. Impact:** A technically capable invitee (e.g. calling Supabase's REST/RPC endpoints directly with their session token instead of going through the app UI) can skip `/confirm-age` entirely and use every other RLS-permitted table/RPC — create trips, chat, publish posts — while remaining permanently unverified. For an invite-created account specifically, this defeats the exact regulatory goal (COPPA / UK AADC / GDPR) the whole age-gate/Hitchhiker design exists for: an unverified, potentially-under-18 identity becomes and stays a full, independent data subject. Direct signup (the primary path) is unaffected — this is scoped to the invite path only.
- **3. Remediation:** a reusable `public.require_age_verified()` `BEFORE INSERT OR UPDATE` trigger, checking `profiles.age_verified_at is not null` for the calling `auth.uid()` (no-ops when `auth.uid()` is null — service-role/migration-context writes, and Hitchhiker token-RPC writes, which were never `auth.uid()`-bearing sessions in the first place). Attached to every table where writing means acting as a full, independent data subject: `itineraries`, `messages`, `itinerary_posts`, `post_likes`, `follows`, `post_comments`, `expenses`, `ratings`, `packing_items`, `trip_segments`. Deliberately not attached to `public.profiles` itself (would make it impossible to ever clear the gate) or to `DELETE` (removing your own data isn't the regulated activity). Applies retroactively/immediately rather than new-accounts-only — safe to do because `stage44_age_gate.sql` already grandfathered every pre-existing account (`age_verified_at = created_at` backfill) and direct signups are stamped verified at profile creation, so in practice this only ever blocks the narrow window the finding is actually about: an invite-created account mid-flow.
- **4. Resolution Mechanism:** DB-level trigger, verified safe-to-re-run (`CREATE OR REPLACE` / `DROP TRIGGER IF EXISTS` throughout) — not yet run against the live database, see the deployment-gaps list at the top of `docs/Checklist.md`. Future-proofed by `scripts/check_age_gate_coverage.sh` (run via `scripts/hooks/pre-commit`), which fails a commit that adds a new Supabase table with neither the trigger nor an explicit `-- age-gate-exempt: public.<table> — <reason>` comment in the same migration file.

---

### [SEC-034] Hitchhiker token-authenticated RPCs have no rate limit

- **Category:** Abuse / Anti-Cheat
- **Severity:** Low
- **Status:** ✅ Accepted risk (2026-08-12, documented at ship time in `stage45_hitchhikers.sql`) — same posture as SEC-021's like/follow deferral, logged here so it's tracked in this doc's registry rather than only in migration comments.
- **Location:** `hitchhiker_send_message(p_token, p_content)`, `hitchhiker_suggest_item(p_token, p_title, p_description)` (`stage45_hitchhikers.sql`) — both `SECURITY DEFINER`, granted to `anon`, no throttle beyond the app's own UI.
- **1. Cause:** Once a valid, unrevoked `access_token` is known, nothing bounds how many messages/suggestions can be sent through it per unit time.
- **2. Impact:** Low. `access_token` is a random `uuid` (122 bits of entropy — not guessable) shared only with the one person the Captain invited, and instantly revocable via `revoke_hitchhiker`. A malicious holder of a legitimate token could spam a trip's chat/suggestion queue, but can't affect any other trip and can be cut off immediately.
- **3. Remediation:** A windowed `count(*)` trigger on `messages`/`itinerary_suggestions` keyed by `hitchhiker_id`, same pattern as stage33/41 — revisit if abuse is actually observed; not pre-built for a risk this narrow.
- **4. Resolution Mechanism:** Accepted, not fixed — the token's entropy + instant revocability is judged sufficient mitigation for the current, narrow exposure.
...
| SEC-033 | Invite-path age gate enforced app-side only, not by RLS/RPC | Medium |
| SEC-034 | Hitchhiker token RPCs have no rate limit | Low |

**Still open:** SEC-014 (Firebase key rotation — manual console action, see Active Remediation Log above). SEC-033 (invite-path age-gate DB enforcement) resolved 2026-08-15 and confirmed live — `stage46_age_gate_db_enforcement.sql` has been run against the production database.
