...
void main() {
  ...
  test('geocodes each leg and inserts a resolved segment in order', () async {
    when(() => geocoder.search('Munich')).thenAnswer(
      (_) async => const [
        GeocodingResult(
          name: 'Munich, Germany',
          latitude: 48.1351,
    ...
    when(() => geocoder.search('Bangkok')).thenAnswer(
      (_) async => const [
        GeocodingResult(
  ...
  test('numbers consecutive resolved legs 0, 1, 2, ...', () async {
    when(() => geocoder.search(any())).thenAnswer(
      (_) async => const [
  ...
  test('skips a leg whose origin cannot be geocoded, without breaking the '
    ...
    when(() => geocoder.search('Nowhere')).thenAnswer((_) async => const []);
    when(() => geocoder.search('Bangkok')).thenAnswer(
      (_) async => const [
        GeocodingResult(
          name: 'Bangkok',
    ...
    when(() => geocoder.search('Chiang Mai')).thenAnswer(
      (_) async => const [
        GeocodingResult(
          name: 'Chiang Mai',
          latitude: 18.7883,
