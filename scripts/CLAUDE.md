...
It copies `scripts/hooks/*` into `.git/hooks/` and makes them executable. The `pre-commit` hook fails the commit if `dart format --set-exit-if-changed lib/ test/` finds unformatted files, `flutter analyze --no-fatal-infos` reports any warning/error, or `check_age_gate_coverage.sh` (below) fails. Two deliberate deviations from the "obvious" invocation: `dart format` is scoped to `lib/`/`test/`, not `.`, because the repo root also contains the gitignored `build/` directory once you've run a native build, and that holds CocoaPods-vendored copies of pub packages (e.g. `firebase_messaging`) that aren't this project's source to format; `--no-fatal-infos` is required because `flutter analyze` is otherwise fatal on info-level issues too, and this project knowingly tolerates a baseline of those (e.g. the deliberate `one_member_abstracts` pattern, see [[feedback-single-method-abstractions]]).

### Age-Gate Coverage Check (SEC-033 future-proofing)

**File:** `scripts/check_age_gate_coverage.sh` — added 2026-08-15 alongside `docs/supabase_migrations/stage46_age_gate_db_enforcement.sql`, which attached a `require_age_verified()` trigger (blocks INSERT/UPDATE from an authenticated-but-unverified invite-path account — see that migration's header for the full SEC-033 writeup) to every table where that mattered at the time. Nothing else in this solo-dev, no-CI project (see root `CLAUDE.md`) would catch a *future* table quietly missing the same guard, so this script is that catch, run from `pre-commit`.

It scans staged/modified `docs/supabase_migrations/*.sql` files for new `create table public.X` statements and fails the commit unless the same file also either attaches the trigger to `X` or carries an explicit `-- age-gate-exempt: public.X — <reason>` comment. Deliberately scoped to staged files only, not the whole migration history — tables from before this convention existed aren't retroactively required to annotate themselves. Table-name extraction and the trigger check are both plain `grep`/`awk` (portable to macOS's BSD toolchain — no GNU-only flags), matching this codebase's own lowercase-SQL-keyword convention rather than attempting case-insensitive matching.
