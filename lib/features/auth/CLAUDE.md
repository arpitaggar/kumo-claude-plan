...
- **`presentation/providers/age_gate_provider.dart`** — `AgeGateNotifier`/`ageGateProvider`, mirrors `OnboardingNotifier`'s pattern (`lib/features/onboarding/presentation/providers/onboarding_provider.dart`) but is server-derived (`profiles.age_verified_at` via `userProfileRepositoryProvider`), not device-local — this is a real authorization gate, not a UI-guidance flag, and a device-local value would be trivially bypassable by reinstalling the app.
