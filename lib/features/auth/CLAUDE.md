...
- **`presentation/providers/age_gate_provider.dart`** — `AgeGateNotifier`/`ageGateProvider`, mirrors `OnboardingNotifier`'s pattern (`lib/features/onboarding/presentation/providers/onboarding_provider.dart`) but is server-derived (`profiles.age_verified_at` via `userProfileRepositoryProvider`), not device-local — a device-local value would be trivially bypassable by reinstalling the app. **This is a UX gate, not the security boundary** — it exists so a legitimate invitee is routed to `/confirm-age` instead of hitting opaque RLS/trigger errors, but the real enforcement for the invite-path window (2026-08-15, `docs/SECURITY_AUDIT.md` SEC-033) is `require_age_verified()`, a DB-level `BEFORE INSERT OR UPDATE` trigger (`docs/supabase_migrations/stage46_age_gate_db_enforcement.sql`) on every write path that matters — a scripted/API client bypassing this screen entirely now gets rejected at the database, not just hidden by the UI, the same posture as the direct-signup trigger.
